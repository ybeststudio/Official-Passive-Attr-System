...
void ShutdownOnFatalError()
{
	if (!g_bShutdown)
	{
		...
		{
			SendNotice(LC_STRING("A critical server error has occurred. The server will restart automatically."));
			SendNotice(LC_STRING("You will be disconnected automatically in 10 seconds."));
			SendNotice(LC_STRING("You can connect again after 5 minutes."));
		}
...
#if defined(__TREASURE_HUNT__)
		CTreasureHuntManager::instance().Process();
#endif
	}
...
int main(int argc, char** argv)
{
	...
	*/
...
#endif
#if defined(__NPC_LOCATION_HELPER__)
	CNpcLocationHelperManager npc_location_helper_manager;
#endif
#if defined(__TREASURE_HUNT__)
...
#if defined(__NPC_LOCATION_HELPER__)
	npc_location_helper_manager.Initialize();
#endif
	fishing::Initialize();
...
#if defined(__MCP_SERVER_AI__)
	// MCP bridge listener. No-op when MCP_PORT is 0/unset.
	if (!mcp_bridge::Start())
	{
		sys_err("MCP: bridge failed to start; aborting startup");
		CleanUpForEarlyExit();
		return 1;
	}
#endif
	...
	g_bNoMoreClient = true;
#if defined(__MCP_SERVER_AI__)
	sys_log(0, "<shutdown> Stopping MCP bridge...");
	mcp_bridge::Stop();
#endif
...
#if defined(__MCP_SERVER_AI__)
	// Service queued MCP bridge requests on the main thread so every handler
	// sees a quiescent game state.
	mcp_bridge::Poll();
#endif
