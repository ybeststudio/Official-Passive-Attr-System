...
void CInputDB::PlayerLoad(LPDESC d, const char* data)
{
	...
	if (!SECTREE_MANAGER::instance().GetValidLocation(pTab->lMapIndex, pTab->x, pTab->y, lMapIndex, pos, d->GetEmpire()))
	{
		...
		// Warp to empire village if location doesn't exist.
		lMapIndex = EMPIRE_START_MAP(d->GetAccountTable().bEmpire);
		pos.x = EMPIRE_START_X(d->GetAccountTable().bEmpire);
		pos.y = EMPIRE_START_Y(d->GetAccountTable().bEmpire);
...
void CInputDB::SafeboxLoad(LPDESC d, const char* c_pData)
{
	...
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_STRING("????????? ?????????????? ????? ?????? ???????."));
		...
		if (item->GetType() == ITEM_PET)
		{
			...
			case PET_UPBRINGING:
			{
				...
				if (lPetDuration <= 0)
				{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("%s is currently fast asleep."), item->GetName());
				}
			}
			break;
...
void CInputDB::AddMonarchMoney(LPDESC d, const char* data)
{
	...
	{
		if (number(1, 100) > 95)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("???? %s ???????? %u ?? ???? ??????", EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire)));
	}
}
...
void CInputDB::DecMonarchMoney(LPDESC d, const char* data)
{
	...
	if (ch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("???? %s ???????? %d ?? ???? ??????", EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire)));
	}
}
...
{
	char notice[256];
	snprintf(notice, sizeof(notice), LC_STRING("%s?? ????? %s ?????? ???????????.", EMPIRE_NAME(info->bEmpire), info->szName));
	SendNotice(notice);
}
