...
DWORD CPythonNetworkStream::GetEmoticonTypeCount() const
{
	return static_cast<DWORD>(m_EmoticonTypeVector.size());
}
...
void CPythonNetworkStream::SendEmoticon(UINT eEmoticon)
{
	...
	{
		TraceError("CPythonNetworkStream::SendEmoticon out of range index %u (registered %u)",
			eEmoticon, static_cast<UINT>(m_EmoticonTypeVector.size()));
		return;
	}
